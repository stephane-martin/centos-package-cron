...
class ReportProducer(object):
    ...
    def __init__(self,
        ...
        self.pkg_fetcher = pkg_fetcher or PackageFetcher(
            ChangeLogParser(), self.executor, repos_to_exclude_list,
            repos_to_include_list
        )
    ...
    def _get_sorted_relevant_advisories(self):
        security_advisories = filter(
            lambda adv: adv['advisory'].type == ErrataType.SecurityAdvisory,
            self.checker.findAdvisoriesOnInstalledPackages()
        )
        if self.skip_old_notices:
            only_advisories = map(
                lambda advpkg: advpkg['advisory'],
                security_advisories
            )
            self.annoyance_check.remove_old_advisories(only_advisories)
            security_advisories = filter(
                lambda advpkg: self.annoyance_check.is_advisory_alert_necessary(advpkg['advisory']),
                security_advisories
            )
    ...
    def _send_advisories_to_syslog(self, security_advisories):
        for advisory_and_package in security_advisories:
            ...
            packages = [
                ("%s|%s|%s" % (pkg.name, pkg.version, pkg.release), pkg)
                for pkg in packages
            ]
    ...
    def _add_advisories_to_email(self, security_advisories, email_body):
        ...
        for advisory_and_package in security_advisories:
            ...
            associated_package_labels = map(
                lambda pkg: "* %s-%s-%s" % (pkg.name, pkg.version, pkg.release),
                advisory_and_package['installed_packages'])
            # Remove dupes
    ...
    def _get_general_updates(self):
        ...
        if self.skip_old_notices:
            general_updates = filter(
                lambda pkg: self.annoyance_check.is_package_alert_necessary(pkg),
                general_updates)
    ...
    def _add_changelogs_to_email(self, general_updates, email_body):
        if len(general_updates) > 0:
            changelogs = map(
                lambda pkg: {
                    'name': pkg.name,
                    'changelog': self.pkg_fetcher.get_package_changelog(pkg.name,pkg.version,pkg.release)
                },
                general_updates
            )
            ...
            for update in general_updates:
                changelog_entry = next(cl for cl in changelogs
                                       if cl['name'] == update.name)
