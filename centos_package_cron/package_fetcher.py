...
class ChangeLogParser(object):
    ...
    def get_log_version_nums(self, version, release):
        versions = [
            self.get_log_version_num_with_release_suffix(version, release),
    ...
    def get_regexes(self, name, version, release):
        return map(
            lambda pat: re.compile(pat, re.MULTILINE | re.DOTALL),
            self.get_regex_patterns(name, version, release)
        )
    ...
    def parse(self, output, name, version, release):
        ...
        if match == None:
            return "Unable to parse changelog for package %s version %s release %s" % (
...
class PackageFetcher(object):
    ...
    def __init__(self,
        ...
        self.repos_to_exclude = repos_to_exclude or []
        self.repos_to_include = repos_to_include or []
    ...
    def fetch_installed_packages(self):
        ...
        result = map(
            lambda x: Package(x.name, x.version, x.release, x.arch, x.repoid),
            packages
        )
        return result
    ...
    def get_what_depends_on(self, name):
        ...
        packages = map(
            lambda pkgStr: PackageParser.parsePackage(pkgStr),
            packages
        )
        return map(
            lambda pkgHsh: Package(pkgHsh['name'], pkgHsh['version'], pkgHsh['release'], pkgHsh['arch'], repository=""),
            packages
        )
